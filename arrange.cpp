...
namespace arrangement {
	...
	//Node
			node::node(const uint16_t p_address, const bool isPattern) : 
				blocks::block_t(p_address), 
				nodePattType(isPattern ? nodeType::pattern : nodeType::node)
				{}
			
			node::node(const bool isPattern){
				// lg("nodeConstr1");
			...
			bool node::isValidPattern() const{
				return address != zeroPointerReplacer;
			}
			
			node node::getNodeAtPosition(const int position) const {
			...
			void node::setNodeAtPosition(const int position, const node child){
				// lg(1);
			...
			const char* node::showType(){
				return nodePattType == nodeType::pattern ? "patt" : "node";
			}
			
			patMem::pattern_t node::getPattern() const{
	...
	nodeIndex getIndexes(const int timelinePosition, const bool logging){
		nodeIndex output;
		if(logging){
			//remlog-lg("getIndex");
		}
	...
	//Arrangechannel
			arrangeChannel::arrangeChannel() : firstPattern({zeroPointerReplacer, true}){}

			patMem::pattern_t arrangeChannel::getCurrentPattern (const int timelinePosition){
				//timeAtStart = micros();
			...
			void arrangeChannel::addPattern (const int timelinePosition, const patMem::pattern_t patt){//Check indexes
				// lgc("addP:");
				// lg(timelinePosition);
			...
			void arrangeChannel::deletePatternIfFound(const patMem::pattern_t patternToDelete){
			...
			node arrangeChannel::getFirstPattern(){
				return firstPattern;
			}
			
			void arrangeChannel::setFirstPattern(const node first){
				firstPattern = first;
			}
			
			void arrangeChannel::removePattern(const int timelinePosition){//FINISH THIS and check it's right
	...
	arrangeChannel& getArrangeChannel(const int arrangement, const int track){
		return arrangeChannels[arrangement][track];
	}
